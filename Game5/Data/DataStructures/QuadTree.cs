...
using Game5.Data.DataStructures.Helper;
...
namespace Game5.Data.DataStructures
{
    ...
    public class QuadTree<T> where T : class, IQuadObject
    {
        ...
        /// </summary>
        private readonly Dictionary<T, QuadNode<T>> objectToNodeLookup = new Dictionary<T, QuadNode<T>>();
        ...
        {
            var bounds = quadObject.GetBounds();
            if (Root == null)
            {
            ...
            while (!Root.Bounds.Contains(bounds)) ExpandRoot(bounds);
        ...
        public List<T> Query(Rectangle bounds)
        {
            ...
            if (Root != null) Query(bounds, Root, results);
            return results;
        }
        ...
        private void Query(Rectangle bounds, QuadNode<T> node, List<T> results)
        {
            ...
            if (bounds.Intersects(node.Bounds))
            {
                foreach (var quadObject in node.Objects)
                    if (bounds.Intersects(quadObject.GetBounds()))
                        results.Add(quadObject);

                foreach (var childNode in node.Nodes) Query(bounds, childNode, results);
            }
        }
        ...
        private void ExpandRoot(Rectangle newChildBounds)
        {
            ...
            var newRoot = new QuadNode<T>(newRootBounds);
        ...
        private void InsertNodeObject(QuadNode<T> node, T quadObject)
        {
            lock (thisLock)
            {
                if (!node.Bounds.Contains(quadObject.GetBounds()))
                ...
                if (!node.HasChildNodes() && node.Objects.Count + 1 > maxObjectsPerLeaf)
                {
                    ...
                    foreach (var childNode in node.Nodes)
                    {
                        ...
                        if (childNode.Bounds.Contains(childObject.GetBounds())) childrenToRelocate.Add(childObject);
                    }
                ...
                foreach (var childNode in node.Nodes)
                    if (childNode != null)
                        if (childNode.Bounds.Contains(quadObject.GetBounds()))
                        {
        ...
        private void quadObject_BoundsChanged(object sender, EventArgs e)
        {
            ...
            {
                var node = objectToNodeLookup[quadObject];
                if (!node.Bounds.Contains(quadObject.GetBounds()) || node.HasChildNodes())
                {
                    RemoveQuadObjectFromNode(quadObject);
                    Insert(quadObject);
                    if (node.Parent != null) CheckChildNodes(node.Parent);
                }
            }
        }
